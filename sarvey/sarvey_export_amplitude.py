...
def exportDataToGisFormat(*, file_path: str, output_path: str, input_path: str,
                          correct_geolocation: bool = False, save_h5: bool = False, logger: Logger):
    ...
    rows = point_obj.coord_xy[:, 1].astype(int)
    cols = point_obj.coord_xy[:, 0].astype(int)

    path_slc = join(input_path, "slcStack.h5")

    with h5.File(path_slc, 'r') as f:
        slc = f['/slc'][:]
    ...
    if save_h5:
        point_amp_obj = Points(file_path=join(dirname(file_path), "p2_amp_ts.h5"), logger=logger)

        point_amp_obj.prepare(point_id=point_obj.point_id,coord_xy=point_obj.coord_xy, input_path=input_path)

        point_amp_obj.phase = amp_db_ts
        point_amp_obj.writeToFile()


def createParser():
    """Create_parser."""
    parser = argparse.ArgumentParser(
        description="Export Amplitude time series from p2 files from '.h5' to GIS data formats.",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""EXAMPLE:
        sarvey_export_amplitude outputs/p2_coh50_ts.h5 -o outputs/shp/p2_amp.shp             # export time series to shapefile
        sarvey_export_amplitude outputs/p2_coh50_ts.h5 -o outputs/shp/p2_amp.gpkg            # export time series to geopackage
        sarvey_export_amplitude outputs/p2_coh90_ts.h5 -o outputs/shp/p2_amp.shp -g          # apply geolocation correction
        sarvey_export_amplitude outputs/p2_coh90_ts.h5 -o outputs/shp/p2_amp.shp -g -t       # skip time series data
        """)
    ...
    parser.add_argument('--save-h5', default=False, action="store_true", dest="save_h5",
                        help='It will save the amplitude as h5 file for sarvey_plot visualization')
...
def main(iargs=None):
    ...
    exportDataToGisFormat(file_path=args.file_path, output_path=args.output_path,
                          ...
                          correct_geolocation=args.correct_geolocation, save_h5=args.save_h5,
                          logger=logger)
