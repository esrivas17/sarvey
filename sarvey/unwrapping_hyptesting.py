...
def objFuncTemporalCoherenceTCoef(x, *args):
    """Compute temporal coherence from parameters and phase. To be used as objective function for optimization.
    ...
    1 - gamma: float
    """
    (design_mat, obs_phase, scale_tcoef) = args
...
def temporalUnwrapping_3v_ttest(*, ifg_net_obj: IfgNetwork, net_obj: Network,  wavelength: float, velocity_bound: float,
    ...
    if num_cores == 1:
        ...
        arc_idx_range, demerr, vel, tcoef, gamma = launchAmbiguityFunction3VarSearchTtest(parameters=args)
    else:
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            results = pool.map(func=launchAmbiguityFunction3VarSearchTtest, iterable=args)
...
def launchAmbiguityFunction3VarSearchTtest(parameters: tuple):
    """Wrap for launching ambiguity function for temporal unwrapping in parallel.
    ...
    for k in range(num_arcs):
        ...
        demerr[k], vel[k], tcoef[k], gamma[k] = searchTemporalCoherence_3v_ttest(
            demerr_range=demerr_range,
            vel_range=vel_range,
...
def searchTemporalCoherence_3v_ttest(*, demerr_range: np.ndarray, vel_range: np.ndarray, tcoef_range: np.ndarray, obs_phase: np.ndarray,
                                  design_mat: np.ndarray):
    """One dimensional search for maximum temporal coherence that fits the observed arc phase.
