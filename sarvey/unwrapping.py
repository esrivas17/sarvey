...
def temporalUnwrappingTunnellingReduced(*, ifg_net_obj: IfgNetworkPiecewise, net_obj: NetworkPiecewise, wavelength: float, velocity_bound: float, vel_excavation_bound: float,
                       demerr_bound: float, num_samples: int, num_cores: int = 1, logger: Logger) -> \
        tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if num_cores == 1:
        ...
        arc_idx_range, demerr, vel_pre, vel_exca, gamma = launchAmbiguityFunctionSearchPiecewiseReduced(parameters=args)
    else:
        logger.info(msg="start parallel processing with {} cores.".format(num_cores))
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            results = pool.map(func=launchAmbiguityFunctionSearchPiecewiseReduced, iterable=args)

        # retrieve results
        for i, demerr_i, vel_pre_i, vel_exca_i, gamma_i in results:
            demerr[i] = demerr_i
            vel_pre[i] = vel_pre_i
            vel_exca[i] = vel_exca_i
            gamma[i] = gamma_i
...
def launchAmbiguityFunctionSearchPiecewiseReduced(parameters: tuple):
    """Wrap for launching ambiguity function for temporal unwrapping in parallel.
    ...
    gamma: np.ndarray
    """
    (arc_idx_range, num_arcs, phase, slant_range, loc_inc, ifg_net_obj, wavelength, velocity_bound, vel_excavation_bound, demerr_bound,
     num_samples) = parameters
    ...
    for k in range(num_arcs):
        ...
        demerr[k], vel_pre[k], vel_exca[k], gamma[k] = oneDimSearchTemporalCoherencePiecewise(demerr_range=demerr_range, 
                                                                            vel_range=vel_range, 
                                                                            vel_excavation_range=vel_excavation_range, 
                                                                            obs_phase=phase[k, :], design_mat=design_mat, 
                                                                            design_mat_pre=design_mat_pre, 
                                                                            design_mat_exca=design_mat_exca, 
                                                                            ix_pre=ix_pre, ix_exca=ix_exca)
        
        prog_bar.update(value=k + 1, every=every,
                        suffix='{}/{} arcs processed. '.format(k + 1, num_arcs))

    return arc_idx_range, demerr, vel_pre, vel_exca, gamma
